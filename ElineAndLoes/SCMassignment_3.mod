...
int barDiscThr = ...;		// threshold for barge discount
float barDiscPer = ...; 	// barge discount percentage
...
dvar float+ v_sss[Factories][TransPts][Periods];		// Tonnes from factory i to transshipment point t via sss in each period p
...
//extra variables for question 3:
dvar boolean barDisc[Factories][TransPts][Periods];		// Tells whether the load is elligible to receive a discount

minimize sum(i in Factories, j in Destinations, p in Periods) (c[j][i]*vc*x[i][j][p])

		+ sum(i in Factories, t in TransPts, p in Periods) 
		(blt[t][i]*v_blt[i][t][p] + swt[t][i]*v_swt[i][t][p] + sss[t][i]*v_sss[i][t][p] + (1 - (barDiscPer/100.0)*barDisc[i][t][p])*bar[t][i]*v_bar[i][t][p])
...
subject to {
  ...
  // bar100 should be either 0 if v_bar < 100 or 1 if v_bar >= 100
  forall(p in Periods)
    forall (i in Factories)
      forall (t in TransPts)
      	barDisc[i][t][p] <= v_bar[i][t][p] / barDiscThr;
}
